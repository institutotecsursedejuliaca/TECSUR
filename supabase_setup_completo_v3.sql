...
--  Script de Configuración Completo (v3) - Tablas, Relaciones, Procedures y Triggers
--  Ejecutar en: Supabase > SQL Editor > New Query en un nuevo proyecto
-- =============================================================================
...
-- 3.4 MÓDULOS (Cada carrera tiene módulos)
...
-- 3.5 CURSOS (Cada curso pertenece a un módulo y tiene un docente asignado)
CREATE TABLE cursos (
  id          UUID     PRIMARY KEY DEFAULT uuid_generate_v4(),
  modulo_id   UUID     NOT NULL REFERENCES modulos(id) ON DELETE CASCADE,
  docente_id  UUID     REFERENCES docentes(id) ON DELETE SET NULL, -- Asignación del docente por curso
  nombre      TEXT     NOT NULL,
  descripcion TEXT,
...
CREATE INDEX IF NOT EXISTS idx_cursos_docente_id ON cursos(docente_id);
...
-- Para asegurar el acceso y simplicidad del frontend:
...
-- NOTA: Opcionalmente, para desactivar RLS en entorno de pruebas/desarrollo local:
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 7. REGISTRO DEL PRIMER USUARIO ADMINISTRADOR (COMENTADO PARA EVITAR CONFLICTOS DE ESQUEMA)
-- ─────────────────────────────────────────────────────────────────────────────
-- NOTA: La inserción directa en las tablas de 'auth' de Supabase (auth.users, auth.identities)
-- está desaconsejada porque las columnas internas varían según la versión de Supabase y 
-- causan el error "Database error querying schema" debido a valores NULL.
--
-- Se recomienda crear el usuario administrador utilizando la API de Autenticación de Supabase
-- o ejecutando el script proporcionado 'node create-admin.js'.
--
-- DO $$
-- DECLARE
--   new_uid UUID := gen_random_uuid();
-- BEGIN
--   IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'institutotecsursedejuliaca@gmail.com') THEN
--     -- 7.1 Insertar usuario principal en auth.users
...
--       'authenticated',
--       'authenticated',
--       'institutotecsursedejuliaca@gmail.com',
--       crypt('huaraya_2018', gen_salt('bf', 10)),
--       NOW(),
...
--       new_uid,
--       new_uid,
--       json_build_object('sub', new_uid, 'email', 'institutotecsursedejuliaca@gmail.com'),
--       'email',
--       new_uid::text,
--       NULL,
--       NOW(),
